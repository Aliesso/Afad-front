  ...
  const tabLinks = [
    ...
    {
      id: 6,
      title: "Elmi və pedaqoji kadrların hazırlanması",
    },
    {
      id: 7,
  ...
  const creativityInfo = document.querySelector(".creativity_info");
  // console.log(creativity_text);
  ...
  //TABBAR
  const tabBar = document.querySelector(".creativity_tabbar");
  tabLinks.map((item) => {
    tabBar.innerHTML += `<li id='${item.id}' class='creativity_tabbarItem'>${item.title}</li>`;
  });

  const tabContent = document.querySelector(".creativity_content");
  tabViews.map((item) => {
    tabContent.innerHTML += `<p class="creativity_text">${item.content}</p>`;
  });
  ...
  const creativity_tabbar = document.querySelector(".creativity_tabbar");
  ...
  [...tabBar.children].forEach((item) => {
    item.addEventListener("click", (e) => {
      ...
      tabViews.forEach((tab) => {
        if (tabLinkId == tab.id) {
          creativityInfo.innerHTML = `
          
          <div class="creativity_content">
            <p class="creativity_text active">${tab.content}</p>
            <p class="creativity_more">Ətraflı oxu</p>
          </div>
          <div class="creativity_image">
            <img
              src="${tab.img}"
              alt=""
            />
